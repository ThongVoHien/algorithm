...
// Template from http://comscigate.com/Books/contests/icpc.pdf
...
{
	double sg = (p1.x-p3.x)*(p2.y-p3.y)-(p1.y - p3.y)*(p2.x-p3.x); if(fabs(sg)<EPS) return 0;
	if(sg>0)return 1;
	return -1;
}
...
{
	double sg = (p1.y - p3.y)*(p2.x-p3.x)-(p1.x-p3.x)*(p2.y-p3.y); //watch range of the numbers
	if(fabs(sg)<EPS){
...
void vex(vector<PT>& vin,vector<PT>& vout) {
	...
	vector<int> used;
	// used[i] is the index of the i-th point on the hull used.push_back(st);
...
void vex2(vector<PT> vin,vector<PT>& vout){
	...
	vout.clear();
	int n=vin.size(); sort(vin.begin(),vin.end());
	PT stk[MAX_SIZE];
	int pstk, i;
	// hopefully more than 2 points stk[0] = vin[0];
	stk[1] = vin[1];
	pstk = 2;
	...
	for(i=1; i<pstk-1; i++) {
		stk[i].x= -stk[i].x; // don’t forget rotate 180 d back. stk[i].y= -stk[i].y;
		vout.push_back(stk[i]);
	} 
}
...
double height(PT a, PT b, PT c){ // height from a to the line bc
	double s3 = dist(c, b); 
...
// two lines given by p1->p2, p3->p4 r is the intersection point // return -1 if two lines are parallel
{
	...
	ua /= d;
	// ub = (p2.x - p1.x)*(p1.y-p3.y) - (p2.y-p1.y)*(p1.x-p3.x); //ub /= d;
	r = p1 + (p2-p1)*ua;
	return 0;
}
...
int hcenter( PT p1, PT p2, PT p3, PT& r ) {
	...
	if( triarea( p1, p2, p3 ) < EPS PT a1, a2) 
		return -1;
...
// in a triangle with sides a,b,c, the angle between b and c // we do not check if a,b,c is a triangle here
{
...
// the relation of the point and the simple polygon // 1 if p is in pv; 0 outside; -1 on the polygon
{
	int i, j;
	int n=pv.size(); 
	pv.push_back(pv[0]); 
...
void show(vector<PT>& p) {
	int i,n=p.size(); 
	for(i=0;i<n;i++) show(p[i]); cout<<":)"<<endl;
}
...
int PInterP(vector<PT>& p1, vector<PT>& p2, vector<PT>& p3) {
	...
	for(i=0;i<n;i++)
		if(pAndPoly(p1, p2[i])!=0) 
			pts.push_back(p2[i]); 
	if(m>1 && n>1) for(i=0;i<m;i++) for(j=0;j<n;j++)
